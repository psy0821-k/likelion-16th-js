...
const psy = createPerson('싸이', 40, '노래하기')
const jyp = createPerson('박진영', 50, '춤추기')

console.log(psy);
console.log(jyp);

const creatHuman = (name, age, job)=>{
  const personObject = {
    name : name,
    age : age,
    job : job,
...
const gd = creatHuman('지드래곤', 35, '아이돌')
...
const createHouse = (houseName, houseType, houseSize, builtIn)=>
( 
  {
    houseName: houseName,
    houseType : houseType, 
...
const richMan = creatHuman("지하철", "오픈형 공유 주택", 1000, true)
